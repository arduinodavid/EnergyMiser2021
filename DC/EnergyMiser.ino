...
/*Versions
  ...
  DC17 - adds scroling message to web page
...
// Create an Event Source on /events DC17
...
char timeToGo[100], msg1[50], msg2[50]; // DC20
    ...
    if (loadIsOn) {
      if (systemState == STATE_WAITING_FOR_LOAD) {
        ...
        if (systemState == STATE_WAITING_FOR_LOAD) { // still waiting
          ...
          sprintf(timeToGo, " %02d:%02d", minsToStart, secsToStart); // DC20
          strcpy(msg2, ""); // DC20
          gPrint(msg1); delay(20);
          gPrint(timeToGo); delay(20); //Serial.println(strMsg); // DC05
          gPrint(msg2); delay(20);
        }
      }
        ...
        if (loadNow < (int)threshold + 5) {
          ...
          sendMessage(String(strMsg)); // DC17
        }
      }
        ...
        if (secsToOff < 0) { // AH15
          ...
          if (minsToOff < 0) {
            ...
            setColor(ORANGE);
            gPrint("Finished!"); Serial.print("Finished!");
            sendMessage("Finished"); // DC17
          }
        }
        ...
        if (systemState == STATE_WAITING_OFF) {
          setTextPosAbs(0, line3); delay(20);
          setColor(WHITE); delay(20);
          strcpy(msg1, "Power OFF in"); // DC20
          sprintf(timeToGo, " %02d:%02d", minsToOff, secsToOff); // DC20
          strcpy(msg2, "(On button to restart timer)"); // DC20
